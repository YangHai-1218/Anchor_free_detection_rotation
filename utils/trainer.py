...
class Trainer:
    ...
    def train(self, model, epoch, optimizer, scheduler=None, logger=None, ema=None):
        ...
        for idx, (images, targets, _) in pbar:
            ...
            loss_quality = loss_dict['loss_quality'].mean()
            #loss_center = loss_dict['loss_centerness'].mean()
            loss_angle = loss_dict['loss_angle'].mean()

            loss = loss_cls + loss_box + loss_quality + loss_angle
            ...
            loss_quality = loss_reduced['loss_quality'].mean().item()

            if get_rank() == 0:
                pbar.set_description(
                    (
                        f'epoch: {epoch + 1}; cls: {loss_cls:.4f}; '
                        f'box: {loss_box:.4f}; iou: {loss_quality:.4f} '
                ...
                if logger and idx % 50 == 0:
                    ...
                    logger.add_scalar('training/loss_quality', loss_quality, totalStep)
                    logger.add_scalar('training/loss_angle', loss_angle, totalStep)
                    logger.add_scalar('training/loss_all', (loss_cls + loss_box + loss_quality), totalStep)
                    logger.add_scalar('learning_rate', lr_rate, totalStep)

            epoch_loss.append(float(loss_cls + loss_box + loss_quality))
        if logger:
...
class Tester:
    def __init__(self,args, loader, dataset, device):
        ...
        self.score_threshold_for_f1 = args.score_threshold_for_f1
        if self.multi_scale_test:
            self.multi_scale_tester = Multi_Scale_Test(self.dataset,
                                                  ...
                                                  nms_threshold=args.nms_threshold_mulit_scal,
                                                  ...
                                                  normalize =Normalize(args.pixel_mean, args.pixel_std))
    ...
    def __call__(self,model,epoch,logger=None,ema=None):
        ...
        for idx, (images, targets, ids) in pbar:
            ...
            pred = [p.to('cpu') if (p is not None) else None for p in pred]

            preds.update({id: p for id, p in zip(ids, pred) if p is not None})
        ...
        evl_res = evaluate(self.dataset, preds, self.result_file, self.score_threshold_for_f1, epoch)
